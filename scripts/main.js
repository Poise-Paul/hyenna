function init() {
  for (
    container = document.getElementById("particles"),
      ...
      tog = !0,
      list = [],
      w = canvas.width = COLS * SPACING + 2 * MARGIN,
      h = canvas.height = ROWS * SPACING + 2 * MARGIN,
      i = 0;
    i < NUM_PARTICLES;
    i++
  )
...
function step() {
  if ((stats && stats.begin(), (tog = !tog)))
    for (
      man ||
        ((t = 0.001 * +new Date()),
        (mx = 0.5 * w + Math.cos(2.1 * t) * Math.cos(0.9 * t) * w * 0.45),
        (my =
          0.5 * h +
          Math.sin(3.2 * t) * Math.tan(Math.sin(0.8 * t)) * h * 0.45)),
        i = 0;
      i < NUM_PARTICLES;
      i++
    )
    ...
    for (
      b = (a = ctx.createImageData(w, h)).data, i = 0;
      i < NUM_PARTICLES;
      i++
    )
      (p = list[i]),
        (b[(n = 4 * (~~p.x + ~~p.y * w))] = b[n + 1] = b[n + 2] = COLOR),
        (b[n + 3] = 255);
...
function randomWord() {
  if (!blocked)
    return (
      ...
      autoChangeWords(),
      count++,
      $("#home").find(".frase strong").text(randWord)
    );
}
...
function showgif() {
  if (!blocked) {
    ...
    $moldura.addClass("--active"),
      $elem.css("opacity", 0),
      $gifs.css("background-image", "url(" + img + ")"),
      randomBt(),
      clearTimeout(intervaloWords);
  }
}
...
function autoChangeWords() {
  intervaloWords = setTimeout(randomWord, 1000);
}
  ...
  (blocked = !1),
    ...
    setTimeout(function () {
      randomWord(),
        ...
        $home.find(".hide").removeClass("hide").addClass("animated");
    }, 500);
}
...
function loadImages() {
  ...
  for (var downloadingImage = [], i = 0; i < total; i++)
    downloadingImage.push(new Image()),
      (downloadingImage[i].onload = function () {}),
      (downloadingImage[i].src = baseUrl + "img/home/" + words[i] + ".gif");
}
      ...
      ajustVideo($video),
        ...
        $(window).scroll(function () {
          if (!outAbout) {
            ...
            if (
              (topEquipe > wTop
                ? $videoFull.removeClass("--back")
                : $videoFull.addClass("--back"),
              wWidth > 767)
            ) {
              var hEquipe = $equipe.height(),
                hWindow = $(window).height();
              wTop > topEquipe && hEquipe + topEquipe > wTop + hWindow
                ? ((scrolling = !0), $aboutEquipe.addClass("fixed"))
                : ((scrolling = !1), $aboutEquipe.removeClass("fixed")),
                wTop > topEquipe
                  ? $colR.addClass("bottom")
                  : $colR.removeClass("bottom");
            }
          }
        }),
...
var words = ["creativity", "uniqueness", "technology", "efficiency", "brainwork", "serving", "you!"],
    ...
    Modernizr.touch
      ...
      : ($home.on("mousedown", showgif),
        ...
        $home.on("mouseenter", ".link-landing", function () {
          $(this).hasClass("--left")
            ? $moldura.find(".moldura-landing__left").addClass("--active")
            : $moldura.find(".moldura-landing__right").addClass("--active");
        }),
        $home.on("mouseleave", ".link-landing", function () {
          $moldura
            .find(".moldura-landing__left,.moldura-landing__right")
            .removeClass("--active");
        })),
              ...
              success: function (data) {
                "true" == data
                  ? ($campo.removeClass("--error"),
                    $msg.filter(".--error").removeClass("--show"),
                    $msg.filter(".--success").addClass("--show"),
                    $(form)[0].reset())
                  : "erro-campos" == data
                  ? $campo.addClass("--error")
                  : "erro-duplicado" == data
                  ? ($campo.addClass("--error"),
                    $msg.filter(".--error").addClass("--show"),
                    $(form)[0].reset())
                  : "erro-inserir" == data &&
                    ($campo.addClass("--error"),
                    $msg.filter(".--error").addClass("--show"));
...
var currentWork,
  ...
  animaWorkIn = function (index) {
    ...
    var $text = $currentWork.find(
      ".work__title,.work__tags,.work__ver-projeto"
    );
  ...
  worksAnimaOut = function (nextPage) {
    setTimeout(function () {
      $.fn.fullpage.destroy("all"),
        $works.parallax("disable"),
        ($currentWork = $work.eq(0));
    }, 1500),
      clearTimeout(timer);
  },
  moveActiveLine = function () {
    var top =
      $menuItem.filter(".active").offset().top -
      0.5 * (lineHeight - menuItemHeight);
  ...
  $(document).ready(function () {
    ...
    var currentPage,
      ...
      changePage = function (current, next) {
        "function" == typeof window[current + "AnimaOut"] &&
          window[current + "AnimaOut"](),
      ...
      loadPage = function (url, back) {
        ...
        $.ajax({
          ...
          back || urlChange(url);
          var $container = $(
            '<div id="' + nextPage + '" class="animation-container"/>'
          );
          $container.html(data),
            $("body").append($container),
            "about" != nextPage &&
              "works" != nextPage &&
              (window.instgrm.Embeds.process(), FB.XFBML.parse()),
      ...
      positionMenu = function (page) {
        ...
        $social.removeClass("--hide"),
          ...
          "about" == page
            ? $menu.removeClass("--works --work --hide").addClass("--about")
            : "works" == page
            ? $menu.addClass("--works").removeClass("--about --work --hide")
            : $menu.addClass("--work").removeClass("--about --works --hide"),
          "works" == page || "about" == page
            ? $backWorks.addClass("--hide")
            : $backWorks.removeClass("--hide"),
          ("home" == page ||
            "services" == page ||
            ("about" != page && "works" != page && "work" != id[0])) &&
            ($backWorks.addClass("--hide"), $menu.addClass("hide"));
      },
      lastScroll = 0;
    $(window).scroll(function () {
      ...
      if (
        (top > h || (wW < 768 && top > 0)
          ? ($barraMenu.addClass("--open"),
            ...
            lastScroll < top
              ? ($barraMenu.addClass("--close"),
                $menu.addClass("--close"),
                $backWorks.addClass("--close"))
              : ($barraMenu.removeClass("--close"),
                $menu.removeClass("--close"),
                $backWorks.removeClass("--close")))
          : ($barraMenu.removeClass("--open"),
            $menu.removeClass("--open"),
            $backWorks.removeClass("--open")),
        "about" != currentPage &&
          $("#" + currentPage).find(".navegacao-projetos").length > 0)
      ) {
        var bottom = $("#" + currentPage)
          .find(".navegacao-projetos")
          .offset().top;
        bottom < top + h
          ? ($social.addClass("--lock").css("top", bottom - 55),
            $languages.addClass("--lock").css("top", bottom - 55))
          : ($social.removeClass("--lock").css("top", ""),
            $languages.removeClass("--lock").css("top", ""));
      }
      lastScroll = top;
    }),
    ...
    loadedPages.push(pageInit),
      ...
      $("body").on("click", ".languages__language", function (event) {
        if (!$(this).hasClass("--active")) {
          ...
          "pt" == $(this).data("lang")
            ? ((url = url.replace(baseUrl, baseUrl + "pt/")),
              setCookie("lang", "pt", 365))
            : (url = url.replace(baseUrl + "pt/", baseUrl)),
